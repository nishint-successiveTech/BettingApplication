...
export const resolvers = {
  Query: {
    ...
    getUserMoney: async (_: any, args: { email: string }) => {
      try {
        const res = await axios.get(`${BACKEND_URL}/users/getMoney/${args.email}`);
        return {
          email: args.email,
          money: res.data.money,
        };
      } catch (error: any) {
        throw new Error(error.response?.data?.message || "Failed to fetch user money");
      }
    },
  },
  ...
  Mutation: {
    createCricketMatch: async (_: any, args: any) => {
      const res = await axios.post(`${BACKEND_URL}/cricketMatches/createMatch`, args);
      return res.data;
    },

    updateCricketMatch: async (_: any, args: { id: string; updateData: any }) => {
      const res = await axios.put(
        `${BACKEND_URL}/cricketMatches/updateMatch/${args.id}`,
        args.updateData
      );
    ...
    deleteCricketMatch: async (_: any, args: { id: string }) => {
      const res = await axios.delete(`${BACKEND_URL}/cricketMatches/deleteMatch/${args.id}`);
      return res.data;
    },

    createUser: async (_: any, args: { name: string; email: string; phoneNumber: string; password: string }) => {
      try {
        const res = await axios.post(`${BACKEND_URL}/users/createUser`, args);
        return {
          id: res.data._id,
